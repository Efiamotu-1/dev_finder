    ...
    <div className='bg-white text-[#697C9A] dark:bg-[#1E2A47] dark:text-white px-8 py-5 rounded-lg shadow-lg'>
        ...
        <div className='col-span-2 lg:col-start-2 lg:col-end-3 bg-[#F6F8FF] dark:bg-[#141D2F] py-5 mb-5 flex justify-evenly items-center rounded-lg'>
            <div className='flex flex-col gap-2'>
                ...
                <p className='text-black dark:text-white font-bold text-[22px]'>{public_repos ? public_repos : 0}</p>
            </div>
            <div className='flex flex-col gap-2'>
                <p className='font-normal text-[13px]'>Followers</p>
                <p className='text-black dark:text-white font-bold text-[22px]'>{followers ? followers : 0}</p>
            </div>
            <div className='flex flex-col gap-2'>
                <p className='font-normal text-[13px]'>Following</p>
                <p className='text-black dark:text-white font-bold text-[22px]'>{following ? following : 0}</p>
            </div>
        </div>
        ...
        <div className='grid md:grid-cols-2 col-span-2 lg:col-start-2 lg:col-end-3 gap-5 mb-5'>
            <div className='flex gap-3 items-center'>
                <img src={Pin}/>
                <p className={!location && 'opacity-50'}>{location ? location : "Not Available"}</p>
            </div>
            <div className='flex gap-3 items-center'>
                <img src={Url}/>
                <a href={blog ? blog : ""} target='_blank' className={blog && `hover:underline` || !blog && 'opacity-50'}>{blog ? blog : "Not Available"}</a>
            </div>
            <div className='flex gap-3 items-center'>
                <img src={Twitter}/>
                <p className={!twitter_username && 'opacity-50'}>{twitter_username ? twitter_username : "Not Available"}</p>
            </div>
            <div className='flex gap-3 items-center'>
                <img src={Office}/>
                <p className={!company && 'opacity-50'}>{company ? company : "Not Available"}</p>
            </div>
        </div>
        </div>
    </div>
